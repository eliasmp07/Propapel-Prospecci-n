...
val compositionLocal = compositionLocalOf<PlatformConfiguration>{
    error("No PlatformConfiguration provided")
}
...
@Composable
expect fun providePlatformConfiguration(): PlatformConfiguration
...
fun rememberSwipeableCardState(): SwipeableCardState {
    ...
    val platformConfig = LocalPlatformConfiguration
    val screenWidth = with(LocalDensity.current) {
        platformConfig.current.screenWidth.dp.toPx()
    }
    val screenHeight = with(LocalDensity.current) {
        platformConfig.current.screenHeight.dp.toPx()
    }
    return remember {
